...
import {
    ...
    Plus, Copy, Check, X, GripVertical, Settings, Palette,
...
import { MOCK_FORMS, DEFAULT_DESIGN, FORM_TEMPLATES, PALETTE_FIELDS } from '@/components/forms/formsData';
...
export default function FormBuilderPage() {
    ...
    return (
        <div className="flex h-screen flex-col overflow-hidden">
            {/* ── Header ── */}
            <div className="flex h-11 shrink-0 items-center justify-between border-b border-border bg-card px-4 gap-3">
                <div className="flex items-center gap-2 min-w-0">
                    <Button variant="ghost" size="sm" className="h-7 gap-1.5 text-xs text-muted-foreground shrink-0" onClick={handleBack}>
                ...
                <div className="flex items-center gap-1.5">
                    <Button variant="outline" size="sm" className="h-7 gap-1.5 text-xs" onClick={() => setShowPreview(true)}>
                        <Eye className="h-3.5 w-3.5" />
                        Preview
                    </Button>
                    <Button variant="outline" size="sm" className="h-7 gap-1.5 text-xs" onClick={() => setShowEmbed(true)}>
                        <Code2 className="h-3.5 w-3.5" />
                        Embed
                    </Button>
                    <Button size="sm" className="h-7 gap-1.5 text-xs bg-accent hover:bg-accent/90 text-accent-foreground" onClick={handleSave} disabled={isSaving}>
