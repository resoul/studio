...
import {
    ...
    Sparkles, BarChart2, Eye, ArrowRight, Users, Send,
    TrendingUp, ExternalLink,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { EmbedForm, FormStatus, FormTemplate } from '@/types/forms';
import { MOCK_FORMS, FORM_TEMPLATES } from '@/components/forms/formsData';
...
export default function FormsPage() {
    ...
    const handleNew         = useCallback(() => navigate('/forms/new'), [navigate]);
    const handleEdit        = useCallback((id: string) => navigate(`/forms/${id}`), [navigate]);
    const handleUseTemplate = useCallback((templateId: string) => navigate(`/forms/new?template=${templateId}`), [navigate]);
    ...
    return (
        <div className="flex flex-col h-full overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-border shrink-0">
                <div>
                    <h1 className="text-lg font-semibold text-foreground">Embed Forms</h1>
                    <p className="text-sm text-muted-foreground mt-0.5">
                        Build embeddable sign-up and contact forms for your website.
                    </p>
                </div>
                <Button onClick={handleNew} className="bg-accent hover:bg-accent/90 text-accent-foreground gap-1.5" size="sm">
